...
if __name__ == '__main__':
    ...
    # 设置输出文件夹路径
    img_output_folder = os.path.join(video_dir, video_name)
    # 创建输出文件夹（如果不存在）
    if not os.path.exists(img_output_folder):
        os.makedirs(img_output_folder)

    fps = video_to_frames(src_video_path, img_output_folder)
    ...
    for dirpath, _, filenames in os.walk(img_output_folder):
         for filename in filenames:
            if count % 100 == 0:
                ...
                for face in faces:
                    dst_iamge = swapper.get(src_iamge, face, source_face, paste_back=True)
                    cv2.imwrite(os.path.join(swap_output_folder, f'{filename}.jpg'), dst_iamge)
                count += 1


    images_to_video(img_output_folder, dst_video_path, fps)
